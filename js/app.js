...
(function(){
  ...
  function startPreloaderChart(){
    ...
    // Gap between candle centers (in pixels)
    const SPACING = 34;
    // How many candles to keep on screen at once
    ...
    function newCandle(prevClose){
      // Wander with a slight upward drift so it looks like a chart
    ...
    function shiftAndAppend(){
      // Move every candle left by a small step
      const step = 0.6;
      candles.forEach(c => { c.x -= step; });

      // Remove off-screen left candles
      while (candles.length && candles[0].x < -SPACING * 2){
        candles.shift();
      }

      // Append a new candle off-screen right
    ...
    function draw(){
      ...
      // Accent line on the top for a "loading" feel
      ctx.globalAlpha = 0.4;
    ...
    function loop(){
      if (!running) return;
      frameCounter++;
      // Shift candles every 4 frames for a smooth scroll
  ...
  function boot(){
    ...
    try {
      ...
      // Load ticker with current theme
      ...
      if (document.readyState === 'complete'){
        ...
        // Safety fallback — hide after 4 seconds even if something is slow
      ...
      // Fail-safe: hide preloader if boot crashes
